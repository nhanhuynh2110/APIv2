...
const { Category, CategoryPost, Product, Gallery, Slide } = Models

module.exports = (router) => {
  router.get('/menu', (req, res) => {
    try {
      const categoryPost = (cb) => {
        CategoryPost.find({ isActive: true, isDelete: false }, (err, categories) => {
      ...
      async.parallel({ menu, categoryPost }, (error, data) => {
        if (error) return utility.apiResponse(res, 500, error.toString())
        const dt = {
          menu: data.menu,
          post:  data.categoryPost
        }
        return utility.apiResponse(res, 200, 'Success', dt)
      })
